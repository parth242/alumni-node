...
slideshowRouter.post('/create', async (req, res) => {
    ...
    try {
        ...
        let slideshow;
        if (id) {
            slideshow = await Slideshow_1.default.findOne({ where: { slide_title: slide_title, id: { $not: id } } });
            console.log("user>>>>>>>>>>>>>>>>", slideshow);
        }
        else {
            slideshow = await Slideshow_1.default.findOne({ where: { slide_title: slide_title } });
        }
        if (slideshow) {
