...
workroleRouter.post('/create', async (req, res) => {
    ...
    try {
        ...
        let workrole;
        if (id) {
            workrole = await WorkRole_1.default.findOne({ where: { workrole_name: workrole_name, id: { $not: id } } });
            console.log("user>>>>>>>>>>>>>>>>", workrole);
        }
        else {
            workrole = await WorkRole_1.default.findOne({ where: { workrole_name: workrole_name } });
        }
        if (workrole) {
